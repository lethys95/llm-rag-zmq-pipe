...
class QdrantRAG(BaseRAG):
            ...
            if self.collection_name not in collection_names:
                ...
                self.client.create_collection(
                    ...
                    vectors_config=VectorParams(
                        size=self.embedding_dim, distance=self.distance
                    ),
                )
    ...
    def store(
        ...
        metadata: dict[str, str | float | int | bool | list | None] | None = None,
    ...
    def retrieve_documents_with_embedding(
        ...
        try:
            ...
            for result in response.points:
                ...
                # Create RAGDocument
                doc = RAGDocument(content=text, score=result.score, metadata=metadata)
    ...
    def delete(self, point_ids: list[str]) -> None:
        ...
        try:
            self.client.delete(
                collection_name=self.collection_name,
                points_selector=cast(list[PointId], point_ids),
            )
    ...
    def update_access_count(self, point_id: str) -> None:
        ...
        try:
            ...
            if not payload:
                logger.error("No payload to set")
                return None
            ...
            self.client.set_payload(
                collection_name=self.collection_name, payload=payload, points=[point_id]
            )
