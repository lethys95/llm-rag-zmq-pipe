...
class NudgingAlgorithm:
    ...
    def __init__(
        ...
        logger.info(f"Nudging algorithm initialized with {len(self.sources)} sources")
    ...
    def _retrieve_source_position(self, source_name: str, topic: str) -> float:
        ...
        """
        logger.debug(
            f"Retrieving position from source '{source_name}' for topic '{topic}'"
        )

        return 0.0

    def calculate_nudge(
        self, topic: str, user_position: float, trust_score: float = 0.0
    ) -> NudgeResult | None:
        ...
        try:
            ...
            companion_influence = self.weights.base_companion_influence + (
            ...
            logger.info(dedent(f"""
                Calculated nudge for '{topic}': 
                user={user_position:.3f}, companion={companion_position:.3f} -> 
                {companion_new:.3f} (nudge={nudge:.3f})
            """))
            ...
            logger.error(
                f"Error calculating nudge for topic '{topic}': {e}", exc_info=True
            )
            return None
    ...
    def update_source_effectiveness(
        self, source_name: str, effectiveness: float
    ) -> None:
