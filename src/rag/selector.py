...
class RAGDocument:
    ...
    content: str
    score: float  # Similarity/relevance score from vector search
    metadata: dict[str, Any]  # Additional metadata (timestamp, source, etc.)
...
class RAGSelector:
    ...
    def _calculate_recency_boost(
        ...
        if age_hours < 24:
            ...
            logger.debug(f"Document age: {age_hours:.1f}h, boost: {recency_multiplier:.3f}")
    ...
    def _calculate_adjusted_score(
        ...
        if timestamp:
            ...
            if doc_time:
                recency_multiplier = self._calculate_recency_boost(doc_time, boost_factor, now)
    ...
    def rank_by_relevance(
        ...
        for doc in documents:
            adjusted_score = self._calculate_adjusted_score(doc, recent_boost_factor, now)
    ...
    def format_for_llm(
        ...
        logger.debug(f"Formatted {len(documents)} documents into {len(result)} characters")
