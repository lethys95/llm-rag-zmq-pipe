...
class SentimentAnalysis(BaseModel):
    ...
    memory_owner: str = Field(
        ...,
        description="Identifier for who this memory belongs to (speaker from input)",
    )
    emotional_tone: str | None = Field(
        None, description="Emotional tone (e.g., happy, angry, sad, confused)"
    )
    relevance: float | None = Field(
        None, ge=0.0, le=1.0, description="General impact/importance (0.0 to 1.0)"
    )
    chrono_relevance: float | None = Field(
        None,
        ge=0.0,
        le=1.0,
        description="How quickly this decreases in relevance over time (0.0 to 1.0)",
    )
    context_summary: str | None = Field(
        None, description="Brief description of the specific situation"
    )
    key_topics: list[str] | None = Field(
        None, description="Main topics identified in the message"
    )
