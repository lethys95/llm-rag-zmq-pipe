...
class TaskScheduler:
    ...
    def __init__(self):
        ...
        self.running = False
        self.observers: list[TaskCallback] = []
    ...
    def register_task(
        ...
        if name in self.tasks:
            logger.warning("Task '%s' already registered, updating", name)
        ...
        logger.info("Registered task '%s' with interval %s", name, interval)
    ...
    def add_observer(self, observer: TaskCallback) -> None:
        ...
        logger.debug("Added observer, total: %s", len(self.observers))

    def remove_observer(self, observer: TaskCallback) -> None:
        """Remove an observer.
        ...
        if observer in self.observers:
            ...
            logger.debug("Removed observer, total: %s", len(self.observers))
    ...
    async def _run_scheduler(self) -> None:
        ...
        while self.running:
            try:
                ...
                logger.error("Error in scheduler: %s", e, exc_info=True)
    ...
    async def _execute_task(self, task: ScheduledTask) -> None:
        ...
        logger.info("Executing task '%s' (run #%s)", task.name, task.run_count)
        ...
        try:
            ...
            for observer in self.observers:
                try:
                    ...
                    logger.error(
                        "Observer error for task '%s': %s", task.name, e, exc_info=True
                    )
