...
class ContextInterpreterHandler:
    ...
    def interpret(
        self, query: str, documents: list[RAGDocument], include_metadata: bool = False
    ) -> str:
        ...
        """
        logger.debug(
            "Interpreting %s documents for query: %s...",
            len(documents),
            query[:100],
        )
        ...
        try:
            ...
            logger.info(
                "Context interpretation successful (input: %s docs, output: %s chars)",
                len(documents),
                len(reformulated),
            )
            ...
            logger.error("Error during context interpretation: %s", e, exc_info=True)
    ...
    def _build_prompt(
        self, query: str, documents: list[RAGDocument], include_metadata: bool
    ) -> str:
    ...
    def _format_documents(
        self, documents: list[RAGDocument], include_metadata: bool
    ) -> str:
        ...
        for i, doc in enumerate(documents, 1):
            if include_metadata:
                ...
                formatted_parts.append(f"Document {i} [{meta_str}]:\n{doc.content}\n")
            else:
                # Simple format
    ...
    def interpret_with_sentiment(
        ...
        try:
            ...
            logger.error(
                "Error during sentiment-aware interpretation: %s", e, exc_info=True
            )
            # Fallback to regular interpretation
            return self.interpret(query, documents)

    def _build_sentiment_prompt(
        self, query: str, documents: list[RAGDocument], sentiment_summary: str | None
    ) -> str:
